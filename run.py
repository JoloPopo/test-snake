import argparse

from snake.base.direc import Direc
from snake.game import Game, GameConf, GameMode
from snake.gui import GameWindow


def main():
    dict_solver = {
        "greedy": "GreedySolver",
        "hamilton": "HamiltonSolver",
        "dqn": "DQNSolver",
    }

    dict_mode = {
        "normal": GameMode.NORMAL,
        "bcmk": GameMode.BENCHMARK,
        "train_dqn": GameMode.TRAIN_DQN,
        "train_dqn_gui": GameMode.TRAIN_DQN_GUI,
    }

    parser = argparse.ArgumentParser(description="Run snake game agent.")
    parser.add_argument(
        "-s",
        default="hamilton",
        choices=dict_solver.keys(),
        help="name of the solver to direct the snake (default: hamilton)",
    )
    parser.add_argument(
        "-m",
        default="normal",
        choices=dict_mode.keys(),
        help="game mode (default: normal)",
    )
    args = parser.parse_args()

    conf = GameConf()
    conf.solver_name = dict_solver[args.s]
    conf.mode = dict_mode[args.m]
    print(f"Solver: {conf.solver_name}   Mode: {conf.mode}")

    # Créer et lancer le jeu avec le titre personnalisé
    game = Game(conf)
    window = GameWindow(
        "JOTY's Snake Game",
        conf,
        game._map,
        game,
        game._on_exit,
        (
            ("<w>", lambda e: game._update_direc(Direc.UP)),
            ("<a>", lambda e: game._update_direc(Direc.LEFT)),
            ("<s>", lambda e: game._update_direc(Direc.DOWN)),
            ("<d>", lambda e: game._update_direc(Direc.RIGHT)),
            ("<r>", lambda e: game._reset()),
            ("<space>", lambda e: game._toggle_pause()),
        ),
    )
    if conf.mode == GameMode.NORMAL:
        window.show(game._game_main_normal)
    elif conf.mode == GameMode.TRAIN_DQN_GUI:
        window.show(game._game_main_dqn_train)
        game._plot_history()


if __name__ == "__main__":
    main()
